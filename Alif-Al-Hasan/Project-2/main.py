...
from algorithms.gaussian_mixture import gaussian_mixture_clustering
from algorithms.spectral import spectral_clustering
...
def print_detailed_results(dataset_name, dataset_results):
    ...
    for algo_name, results in dataset_results.items():
        ...
        if details["algorithm"] == "KMeans":
            ...
            details_str = (
                f"k={details['n_clusters']}, cov_type={details['covariance_type']}"
            )
        ...
        with open(results_file, "w") as f:
            ...
            for algo_name, algo_results in results.items():
                ...
                if details["algorithm"] == "KMeans":
                    ...
                    f.write(f"    Number of components: {details['n_clusters']}\n")
...
def main():
    ...
    for dataset_file in dataset_files:
        ...
        print(f"\n--- GaussianMixture ---")
        try:
            labels, details = gaussian_mixture_clustering(features, CONFIG, true_labels)
            metrics = calculate_metrics(true_labels, labels, features)
            dataset_results["GaussianMixture"] = {
                "labels": labels,
                "metrics": metrics,
                "details": details,
            }
        ...
        print(f"\n--- Spectral ---")
        try:
            labels, details = spectral_clustering(features, CONFIG, true_labels)
            metrics = calculate_metrics(true_labels, labels, features)
            dataset_results["Spectral"] = {
                "labels": labels,
                "metrics": metrics,
                "details": details,
            }
